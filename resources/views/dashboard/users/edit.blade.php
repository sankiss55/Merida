        ...
        <div class="container">
            <div class="flex rounded-lg bg-white shadow-lg justify-center">
                <div class="w-full lg:w-7/12 bg-white p-5 rounded-lg lg:rounded-l-none">
                    ...
                    <form class="px-8 pt-6 pb-8 mb-4 bg-white rounded" method="post" action="{{ route('dashboard.users.update',['id'=>$user->id]) }}">
                        @method('PATCH')
                        @csrf
                        <div class="mb-4 md:flex md:justify-between">
                            ...
                            <div class="md:ml-2">
                                ...
                                <select name="rol" id="rol" class="w-full px-3 py-2 text-sm leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline">
                                    <option value="{{ old('rol') }}">--ELEGIR--</option>
                                    @foreach($roles as $rol)
                                        <option value="{{ $rol->id }}" {{ (old('rol',($user->roles()->count())?$user->roles()->first()->id:'') ==$rol->id)?'selected':'' }}>{{ $rol->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
